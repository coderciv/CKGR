...
def entity_ranking_and_hits(args, model, test_triplets, e1_to_multi_e2, e2_to_multi_e1, network, write_results=False, debug=False, logger = None):
	...
	for i in range(0, end, batch_size):
		...
		e1_multi = [torch.LongTensor(e2_to_multi_e1[(e.cpu().item(), r.cpu().item())]) for e, r in zip(e2, rel_reverse)]

		with torch.no_grad():
			pred_e2 = model(e1=e1, rel=rel)
			pred_e1 = model(e1=e2, rel=rel_reverse)

		pred_e2, pred_e1 = pred_e2.data.cpu(), pred_e1.data.cpu()
		...
		for j in range(0,cur_batch_size):
			...
			filter1 = e2_multi[j].long()

			filter2 = e1_multi[j].long()

			target_e2 = pred_e2[j, e2[j].item()].item()
			target_e1 = pred_e1[j, e1[j].item()].item()


			pred_e2[j][filter1] = 0.0
			pred_e1[j][filter2] = 0.0


			pred_e2[j][e1[j].item()] = 0.0
			pred_e1[j][e2[j].item()] = 0.0


			pred_e2[j][e2[j]] = target_e2
			pred_e1[j][e1[j]] = target_e1

			scores_e2.append(target_e2)

		max_values, argsort_e2 = torch.sort(pred_e2, 1, descending=True)
		max_values, argsort_e1 = torch.sort(pred_e1, 1, descending=True)

		for j in range(0,cur_batch_size):

			temp_rank_e2 = (argsort_e2[j] == e2[j]).nonzero().cpu().item()
			temp_rank_e1 = (argsort_e1[j] == e1[j]).nonzero().cpu().item()


			ranks.append(temp_rank_e2+1)
			ranks_e2.append(temp_rank_e2+1)
			ranks.append(temp_rank_e1+1)
			ranks_e1.append(temp_rank_e1+1)

			node_e1 = network.graph.nodes[e1[j].cpu().item()]
			node_e2 = network.graph.nodes[e2[j].cpu().item()]

			if node_e1 not in node_mrr:
				node_mrr[node_e1] = []
			if node_e2 not in node_mrr:
				node_mrr[node_e2] = []

			node_mrr[node_e1].append(temp_rank_e1)
			node_mrr[node_e2].append(temp_rank_e2)

			for hits_level in range(0,50):
				if temp_rank_e1 <= hits_level:
					hits[hits_level].append(1.0)
					hits_e1[hits_level].append(1.0)
				else:
					hits[hits_level].append(0.0)
					hits_e1[hits_level].append(0.0)
